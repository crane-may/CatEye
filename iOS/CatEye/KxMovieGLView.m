...
@implementation KxMovieGLView {
    ...
    float _decoder_frameWidth;
    float _decoder_frameHeight;
...
-(void)setSizeWidth:(float)width height:(float)height{
    _decoder_frameWidth = width;
    _decoder_frameHeight = height;
}
...
- (void)updateVertices
{
    ...
    const float width   = _decoder_frameWidth;
    const float height  = _decoder_frameHeight;
...
- (void)render:(char*) buf length:(int)len
{
    ...
    if (len > 0) {
        ...
        if (got_picture_ptr <= 0) return;
        
        frame = [[KxVideoFrameYUV alloc] init];
        
        frame.width = _decoder_frameWidth;
        frame.height = _decoder_frameHeight;
        frame.luma = copyFrameData(pFrame->data[0],
                                   pFrame->linesize[0],
                                   _decoder_frameWidth,
                                   _decoder_frameHeight);
        frame.chromaB = copyFrameData(pFrame->data[1],
                                      pFrame->linesize[1],
                                      _decoder_frameWidth / 2,
                                      _decoder_frameHeight / 2);
        frame.chromaR = copyFrameData(pFrame->data[2],
                                      pFrame->linesize[2],
                                      _decoder_frameWidth / 2,
                                      _decoder_frameHeight / 2);
    }else{
